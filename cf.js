...
if (process.argv.length <= 2) {
    console.log("Usage: node cf.js <url> <time> [<proxy>]");
    console.log("Usage: node cf.js <http://example.com> <60> [<http://localhost:8000>]");
    process.exit(-1);
}
...
if (typeof proxy !== 'undefined') {
    var cloudscraper = require('cloudscraper').defaults({ 'proxy': proxy });
} else {
...
var int = setInterval(() => {
    ...
    cloudscraper.get(url, function(error, response, body) {
        if (error) {
            console.log('['+getTime()+'] Error occurred: ' +JSON.stringify(error));
        } else {
